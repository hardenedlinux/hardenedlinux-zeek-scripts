...
export {
	...
	global multi_notice_non_block_types: set[Notice::Type] = { #  	 	Test::HTTP_Header_Alert_with_src
	} &redef;

	global watch_hosts: table[addr] of table[Notice::Type] of count
	    &write_expire=120 min;
	...
	global single_notice_threshold: table[Notice::Type] of SNT = { } &redef;
}
...
function watch_host(whost: addr, n: Notice::Info)
{
	...
	if ( whost !in watch_hosts ) {
		local init_wh: table[Notice::Type] of count = {
			[n$note] = 1
		};
		...
		if ( n$note in single_notice_threshold ) {
			if ( watch_hosts[whost][n$note] == single_notice_threshold[n$note]$thres ) {
				NOTICE([
				    $note=Multi::Single_Notice_Threshold,
				    $msg=fmt("Crossed threshold of %d for %s", single_notice_threshold[n$note]$thres, n$note),
				    $src=whost,
				    $identifier=cat(whost)]);
			} else if ( single_notice_threshold[n$note]$block == T
			    && watch_hosts[whost][n$note] == single_notice_threshold[n$note]$blockthres ) {
				NOTICE([
				    $note=Multi::Single_Notice_Threshold_Block,
				    $msg=fmt("Crossed block threshold of %d for %s", single_notice_threshold[n$note]$blockthres, n$note),
				    $src=whost,
				    $identifier=cat(whost)]);
			}
		}
	...
	if ( num_notices >= 1 ) {
		for ( wnote in watch_hosts[whost] ) {
			...
			notice_string_sub = cat(notice_string_sub, wnote, ":",
			    watch_hosts[whost][wnote]);
		}

		if ( num_notices >= multi_non_block_thres ) {
			multi_note = Multi::Multi_Notice_AutoBlockAlarm;
		} else if ( ( wnote in multi_notice_non_block_types )
		    && ( num_notices < multi_non_block_thres ) ) {
