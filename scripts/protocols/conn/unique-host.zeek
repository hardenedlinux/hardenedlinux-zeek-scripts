...
export {
	...
	global epoch: interval = 1 hr &redef;
...
event zeek_init()
{
	...
	Log::create_stream(UniqueHosts::LOG, [
	    $columns=Info,
	    $ev=log_conn_count,
	    $path="unique-host"]);
	...
	SumStats::create([
		...
		for ( i in counttable )
			...
			local rec = [
			    $start_time=strftime("%c", r$begin),
			    $epoch=epoch,
			    $net=key$str,
			    $ip_cnt=r$counttable[i]];
	...
	if ( c$id$orig_h in watched_nets || c$id$resp_h in watched_nets ) {
		...
		for ( net in watched_nets ) {
			if ( c$id$orig_h in net ) {
				SumStats::observe("unique.hosts", [
				    $str=fmt("%s", net)], [
				    $str=fmt("%s", c$id$orig_h)]);
			}
