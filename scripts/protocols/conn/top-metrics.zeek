...
event zeek_init()
{
	...
	Log::create_stream(TopMetrics::URLS, [
	    $columns=Info,
	    $ev=log_top_urls,
	    $path="top_urls"]);
	Log::create_stream(TopMetrics::TALKERS, [
	    $columns=Info,
	    $ev=log_top_talkers,
	    $path="top_talkers"]);
	...
	SumStats::create([
		...
		Log::write(TopMetrics::URLS, [
		    $epoch_time=ts,
		    $top_list=top_list,
		    $top_counts=top_counts]);
	}]);
	SumStats::create([
	    $name="tracking top talkers",
	    $epoch=epoch_duration,
	    $reducers=set(r2),
	    $epoch_result (ts: time, key: SumStats::Key, result: SumStats::Result) = {
...
# 
function generate_talker_observations(id: conn_id, total_bytes: count,
    bytes_left: int, num_iters: int)
{
	...
	SumStats::observe("top.talkers", [ ], [
	    $str=fmt("[%s : %s : %s : %s](%d)", id$orig_h, id$orig_p, id$resp_h, id$resp_p, total_bytes)]);
