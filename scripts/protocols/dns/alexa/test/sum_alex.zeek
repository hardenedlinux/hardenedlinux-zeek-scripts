...
event zeek_init()
{
	...
	$threshold=5.0, # Provide a callback to calculate a value from the result
...

{
	...
	if ( ! ( get_domain in alexa_table )
		...
		SumStats::observe("missed_alexa_dns", SumStats::Key(
		...
		NOTICE([
		    $note=Alexa::DNS_Not_In_Alexa_1M, # FIXME : Need to fix bug that value used but not set
