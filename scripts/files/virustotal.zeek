...
global query_limiter: set[string] &create_expire=1 min;
...
function VirusTotal::parse_result(report: Report, result: string)
{
	...
	local jq_result = Exec::Command(
	    $cmd=fmt("echo '%s' | %s '.' >> virustotal_result.json", result, jq));
		...
		if ( /permalink/ in parts[i] ) {
			...
			for ( part_i in scan_parts ) {
				if ( "scan_date" == scan_parts[part_i] ) {
					...
					report$total_scanners = to_count(gsub(scan_parts[part_i
					    + 1], /[^0-9]/, ""));
				}
			}
		}
...
{
	if ( api_key != "" && |query_limiter| < queries_per_minute ) {
		...
		return when ( local result = ActiveHTTP::request(r) ) {
			local report = Report($ts=network_time(), $fuid=f$id);
			if ( f$info?$mime_type ) {
